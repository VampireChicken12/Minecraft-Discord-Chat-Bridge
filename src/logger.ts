...
export default class Logger {
	...
	constructor(
		...
		const time = `${this._padLeft(
			date.getMonth() + 1 + "",
			2,
			0 + ""
		)}/${this._padLeft(date.getDate() + "", 2, 0 + "")} ${this._padLeft(
			date.getHours() + "",
			2,
			0 + ""
	 ...
	 */
	error(...message: any[]) {
		this._reportError?.(format(...message), "Logger Caught");
		this.logStream?.write(
			`${this._log(
				new Date(),
				"error",
				inspect(message[0], { depth: null }),
				...message.slice(1)
			)}\n\r`
		);
	}
	 ...
	 */
	private _padRight(msg: string, pad: number, padChar = " "): string {
		padChar = `${padChar}`;
	 ...
	 * @return {String} The padded string
	 */
	private _padLeft(msg: string, pad: number, padChar = "0"): string {
